...
suite('Functional Tests', function() {
  ...
  suite('Integration tests with chai-http', function() {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function(){
      ...
      test('Test GET /hello with your name',  function(done){ // Don't forget the callback...
         chai.request(server)             // 'server' is the Express App
          .get('/hello?name=Adam') /** <=== Put your name in the query **/ 
          .end(function(err, res){        // res is the response object
            ...
            assert.equal(res.status, 200);
             assert.equal(res.text, 'hello Adam'/** <==  Put your name here **/);
            done();   // Always call the 'done()' callback when finished.
          });
      });
  ...
  Browser.site = 'https://projekt-captain-clark.c9users.io'; 
  ...
  suite('e2e Testing with Zombie.js', function() {
    ...
    suite('"Famous Italian Explorers" form', function() {
      ...
      test('#example - submit the input "surname" : "Polo"', function(done) {
        ...
        //clickLink is neccessary in c9 because default page has link to actual app, so I need to click link first
        browser.clickLink("a").then(function() {
          browser
          .fill('surname', 'Polo')
